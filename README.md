...
- [应用编排层](#应用编排层)
  - [Options 与 CLI](#options-与-cli)
  - [ServicePlan](#serviceplan)
  - [RunLoop 与信号处理](#runloop-与信号处理)
- [服务层](#服务层)
...
|  |                                                            |  |
|  |  +-------------------+     +---------------------------+  |  |
|  |  | ServicePlan       |     | RTTBridge                 |  |  |
|  |  | Options / RunLoop |     | (CB Discovery & Validate) |  |  |
|  |  +--------+----------+     +---------------------------+  |  |
|  |           |                                               |  |
|  |           v                                               |  |
|  |  +-------------------+     +---------------------------+  |  |
|  |  | CoreLogRecorder   |     | Terminal Service          |  |  |
|  |  |  Linux / RTOS     |     | (Telnet / Console)        |  |  |
|  |  +--------+----------+                                    |  |
|  |           |                                               |  |
|  |           v                                               |  |
|  |  +-------------------+     +---------------------------+  |  |
|  |  | LogCollector      |     | SystemView Service        |  |  |
|  |  |  Linux / RTOS     |     | (Network / Local Record)  |  |  |
|  |  +--------+----------+     +---------------------------+  |  |
|  |           |                                               |  |
|  |           v                                               |  |
|  |  +-------------------+     +---------------------------+  |  |
|  |  | LogMerger         |     | SwimLaneRenderer          |  |  |
|  |  | (Sort by time)    |     | (Display / File)          |  |  |
|  |  +-------------------+     +---------------------------+  |  |
|  +------------------------------------------------------------+  |
|                                                                  |
...
|  +----------------+ +----------------+ +----------------+        |
|  | RTT Channel 0  | | RTT Channel 1  | | RTT Channel 2  |        |
|  | Linux logs     | | RTOS logs      | | SystemView     |        |
|  +----------------+ +----------------+ +----------------+        |
+--------------------^-------------------^-------------------------+
                     |                   |
          +----------+-----------+       +----------+-----------+
          | Linux Domain         |       | RTOS Domain          |
          | Writes ch0 and ch2   |       | Writes ch1 and ch2   |
          +----------------------+       +----------------------+
```
...
RTTBridge
    |
    +----> CoreLogRecorder ---> Terminal Service ---> TCP Client / stdin/stdout
    |              |
    |              +---> linux_*.log / rtos_*.log
    |
    +----> Terminal Service (non-core text channel direct read) ---> TCP Client / stdin/stdout
    |
...
```c
typedef struct {
    unsigned total_width;        // 总显示宽度，0 表示自动解析
...
TraceHub 的应用层位于 `App/`，负责把命令行输入解析为完整服务计划，并把主程序入口、信号处理、初始化顺序和运行循环集中到单一编排路径。上层编排不读取 RTT 数据，也不直接写日志数据文件；它只决定启用哪些服务、各服务使用哪些 channel、日志文件前缀如何生成，以及运行期 fatal 状态如何终止进程。

### Options 与 CLI

**文件**: `App/Options.h` / `App/Options_internal.h` / `App/OptionsApi.c` / `App/OptionsParse.c` / `App/OptionsGetoptWin.c` / `App/CLI.h` / `App/CLI.c` / `App/TraceHubDefaults.h`

Options 模块是运行时参数解析的唯一入口，输出 `TraceHubOptions_t` 表示原始用户意图。解析阶段只做 token、数值范围和后端可用性校验，不推导服务组合；服务组合由 ServicePlan 统一完成。Windows 构建使用 `OptionsGetoptWin.c` 提供 `getopt_long` 兼容实现，避免 CLI 解析逻辑在平台间分叉。CLI 模块只负责 banner、usage 和 version 文本输出，编译期默认值统一来自 `TraceHubDefaults.h` 和 CMake 注入的 `TRACEHUB_...` 宏。

### ServicePlan

**文件**: `App/ServicePlan.h` / `App/ServicePlan.c`

ServicePlan 把 `TraceHubOptions_t` 归一化为 `TraceHubServicePlan_t`，并一次性生成以下运行配置：

```
RTTBridge_Config_t       - RTT 后端路径、搜索地址、搜索大小、超时和 MEMSHM reset
CoreLogRecorder_Config_t - Linux/RTOS 核心日志来源、文件前缀和启用状态
Terminal_Config_t        - Console 或 Telnet 单来源服务配置
SystemView_Config_t      - SystemView channel、本地记录和 TCP 服务配置
TraceHubRunLoop_Config_t - RunLoop 使用的模式、channel、端口和 swimlane 宽度
```

ServicePlan 是模式选择、channel 冲突检查和日志路径前缀生成的单一真相源。它保证 `--console`、`--swimlane`、`--telnet-port` 等互斥模式不会同时进入运行层；保证 Linux/RTOS 单来源选择不会在 Console/Telnet 模式下冲突；保证 SystemView channel 不与任何已启用核心文本日志 channel 复用；并在创建任何日志文件前校验 `--log-dir` 已存在且是目录。

### RunLoop 与信号处理

**文件**: `App/RunLoop.h` / `App/RunLoop.c` / `App/TraceHubSignal.h` / `App/TraceHubSignal.c` / `main.c`

`main.c` 只执行固定生命周期：解析参数、构建 ServicePlan、初始化 LOG/RTT/服务、设置 RTTBridge 运行状态、进入 RunLoop、退出时按相反方向停止和清理服务。`RunLoop.c` 负责区分 Swimlane 与普通服务模式：

- Swimlane 模式初始化 `LogCollector`、`LogMerger` 和 `SwimLaneRenderer`，启动采集线程、核心日志记录器和可选 SystemView 服务，并在运行期定期执行 ready flush。
- 普通模式启动 Terminal、核心日志记录器和 SystemView，持续检查各服务 fatal 状态。
- `TraceHubSignal.c` 持有进程 run flag；`SIGINT` / `SIGTERM` 将 run flag 置零，致命信号只执行 async-signal-safe 错误输出并退出。
...
MEMSHM 默认映射大小包含 RTT control block、所有配置的 up/down buffer pair，以及一个 `SEGGER_RTT_SPINLOCK_SW_SIZE` 大小的共享软件 spinlock 预留区域。TraceHub 文本 simulator 不再创建或使用 SystemView 共享锁；SystemView target 侧仿真由 `embOS/BoardSupport/Simulation` 承接。
...
+----------------+------------------------------------------+------------------------------------------+
| ByteQueue      | Core/ByteQueue.h, Core/ByteQueue.c       | Terminal 和 SystemView live TCP backlog  |
|                |                                          | 使用的固定容量环形字节队列               |
+----------------+------------------------------------------+------------------------------------------+
...
这些模块的所有权边界保持明确：LOG 负责统一文件创建和诊断输出；SYS 只提供主机 OS 抽象；Socket/TelnetCodec 只服务 TCP/Telnet 字节流，不读取 RTT；ByteQueue 只保存所属服务的 live 投递缓存，不承担持久化职责；Utils 只提供路径处理。
...
```
1. TraceHubOptions_Parse()      解析原始命令行参数
2. TraceHubServicePlan_Build()  生成完整服务计划并校验模式/channel/log-dir
3. CLI_PrintBanner()            输出 banner（Console 模式写 stderr）
4. LOG_InitEx()                 主诊断日志文件初始化
5. TraceHubSignal_Init()        注册信号处理器（SIGINT, SIGTERM, SIGILL 等）
6. RTTBridge_Init()             RTT 内存映射、RTTCB 搜索和控制块校验
7. CoreLogRecorder_Init()       已启用核心文本来源的日志文件初始化
8. Terminal_Init()              Terminal 服务状态初始化
9. SystemView_Init()            SystemView 服务状态初始化
10. RTTBridge_SetRunning(true)  设置运行状态
11a. _RunSwimLaneMode()         Swimlane 模式运行循环
     - LogCollector_Init()
     - LogMerger_Init()
     - SwimLane_Init()
     - LogCollector_Start()
     - CoreLogRecorder_Start()
     - SystemView_Start()（仅启用 SystemView 时）
11b. _RunNormalMode()           Console/Telnet/SystemView 运行循环
     - Terminal_Start()
     - CoreLogRecorder_Start()
     - SystemView_Start()
```
...
+--------------------+----------+------------------------------------------+
| ByteQueue          | 否       | 所属服务线程串行访问或由服务状态锁保护    |
+--------------------+----------+------------------------------------------+
...
+--------------------+----------+------------------------------------------+
| Options / CLI      | N/A      | 进程启动阶段解析参数和输出帮助文本        |
+--------------------+----------+------------------------------------------+
| ServicePlan        | N/A      | 进程启动阶段生成只读服务计划              |
+--------------------+----------+------------------------------------------+
| RunLoop            | 依服务   | 只编排服务启动、停止和 fatal 状态检查     |
+--------------------+----------+------------------------------------------+
| TraceHubSignal     | 部分     | run flag 供信号处理器和运行循环共享       |
+--------------------+----------+------------------------------------------+
| Utils              | N/A      | 纯路径处理 helper，无全局状态             |
+--------------------+----------+------------------------------------------+
```

SystemView target 侧多核同时写同一 RTT up-buffer 时必须使用跨核共享锁保护 `SEGGER_SYSVIEW_LOCK()` / `SEGGER_SYSVIEW_UNLOCK()`。TraceHub 只消费 SystemView RTT trace，不再承载 target 侧 SystemView demo；target 侧仿真由 `embOS/BoardSupport/Simulation` 承接。真实芯片环境应在共享内存或硬件互斥资源中放置同一类锁对象，并为每个 core 分配稳定唯一的 core id。
...
MEMSHM smoke 验证的是主机文本日志仿真链路。它能覆盖 tracehub 进程启动、host shared memory 后端、模拟 RTTCB、Linux/RTOS 文本日志和泳道排序，但不能证明真实芯片 SharedMem 驱动、目标端 cache 属性、SystemView target 侧集成或真实 SystemView GUI 兼容性。
...
SEGGER SystemView GUI 兼容性必须用真实 GUI 连接 `--systemview-port` 验收。TraceHub 的 SystemView TCP Hello 语义和网络队列行为由 `test_systemview_api` 单元测试覆盖；target 侧 SystemView 事件源由 `embOS/BoardSupport/Simulation` 验收。
...
Linux、macOS 和 Windows 主机均支持 MEMSHM 仿真后端。SMEM SharedMem 真实芯片后端依赖 Linux SharedMem 驱动，只能在 Linux 主机上运行；macOS 和 Windows 不能运行 SMEM，也不能使用 `Scripts/run-console.sh`、`Scripts/run-terminal.sh`、`Scripts/run-sysview.sh`、`Scripts/run-smem-env-check.sh` 或 `Tests/Scripts/run-smem-preflight.sh` 进行真实芯片路径验证。
...
当前抽象层服务于实际运行路径：Options/CLI 只表达原始用户意图和帮助文本，ServicePlan 是模式解析、channel 冲突检查和日志路径生成的单一真相源，RunLoop 只编排服务生命周期；RTTMemory 区分 MEMSHM 和 SMEM 后端，CoreLogRecorder 保证核心文本日志只有一个 RTT reader，LogLineParser 集中承载日志行解析规则，LogCollector/LogMerger/SwimLaneRenderer 保证泳道采集、排序和渲染分层，Socket/TelnetCodec 只处理 TCP/Telnet 字节流，ByteQueue 只作为所属服务的 live TCP backlog，SystemView 独立读取二进制 trace 通道。新增功能必须遵守这些所有权边界：核心文本日志 RTT up-buffer 只能由 CoreLogRecorder 读取；SystemView trace 只能由 SystemView 服务读取；网络 backlog 不是权威日志，文件记录才是持久化来源；运行模式和 channel 规则必须在 ServicePlan 层集中维护。
...
--test                    构建成功后运行 CTest
--no-test                 只构建，不运行 CTest
--coverage                启用覆盖率插桩、运行单元测试并打印摘要
--no-coverage             禁用覆盖率插桩
-r, --run                 构建成功后运行 tracehub
-h, --help                显示 build.sh 帮助信息
...
```bash
CMAKE_BUILD_TYPE          初始构建类型，默认 Debug
TARGET_ARCH               初始构建目标，默认 host
MEMORY_BACKEND            初始内存后端；host 默认 memshm，linux-arm64 默认 smem
TOOLCHAIN_FILE            linux-arm64 交叉构建使用的 CMake toolchain 文件
COVERAGE                  初始覆盖率开关，取 ON 或 OFF
```
...
# 构建后运行单元测试
./build.sh --backend memshm --test

# 覆盖率构建，运行单元测试并打印 gcov/llvm-cov 摘要
./build.sh --backend memshm --clean --coverage
...
单元测试目标由 `TraceHub/Tests/CMakeLists.txt` 统一维护。当前 host 构建覆盖 LogLineParser、LogCollector、LogMerger、SwimLaneRenderer、Core/ByteQueue/Utils/SYS、Log API、Network/Socket/TelnetCodec、RTT API、MEMSHM 后端、Linux 上的 SMEM 后端、Options、ServicePlan、RunLoop、CoreLogRecorder、Terminal 和 SystemView API。MEMSHM 仿真程序目标为 `rtt_sim_linux` 和 `rtt_sim_rtos`。
...
| Preset | 后端 | 构建类型 | 单元测试 | 仿真程序 |
|--------|------|----------|----------|----------|
| `host-memshm-debug` | MEMSHM | Debug | ON | ON |
| `host-memshm-release` | MEMSHM | Release | ON | ON |
| `host-smem-debug` | SMEM | Debug | ON | OFF |
| `host-smem-release` | SMEM | Release | ON | OFF |
| `linux-arm64-memshm-debug` | MEMSHM | Debug | OFF | OFF |
| `linux-arm64-memshm-release` | MEMSHM | Release | OFF | OFF |
| `linux-arm64-smem-debug` | SMEM | Debug | OFF | OFF |
| `linux-arm64-smem-release` | SMEM | Release | OFF | OFF |
...
`Scripts/run-smem-env-check.sh` 用于在不启动 tracehub 服务模式的情况下检查 SMEM runner 环境。默认行为是解析唯一 SMEM `tracehub` 可执行文件，并要求已加载的 `SharedMem` 模块提供可读写设备节点；加入 `--load-module` 时会加载模块，加入 `--keep-module` 时在脚本退出后保留本次加载的模块。该脚本同样支持 `TRACEHUB_BIN` 显式指定二进制路径。

```bash
# run-smem-env-check.sh 专用或额外参数
--load-module           加载 SharedMem 后验证设备节点
--keep-module           与 --load-module 配合使用，脚本退出后保留模块
--skip-tracehub         不检查 tracehub 可执行文件

# 示例
Scripts/run-smem-env-check.sh --use-loaded-module
sudo Scripts/run-smem-env-check.sh --load-module -k /path/to/SharedMem.ko
```
...
MEMSHM smoke 脚本用于验证主机文本日志仿真链路，不替代单元测试。它按 `tracehub -> rtt_sim_linux -> rtt_sim_rtos` 顺序启动进程，并断言以下结果：
