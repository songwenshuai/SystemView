...
`LOG_LINE_TIMESTAMP_PARSE_EMPTY_CONTENT` 会更新当前 source 的时间戳并触发已暂存无 timestamp 启动日志的交付，但不会为该空内容单独创建 LogEntry。
...
3. Parse Timestamp
   ...
   没有 timestamp prefix 的日志行使用同一 source 的当前 timestamp
...
内部缓冲区片段大小：`LOG_COLLECTOR_MAX_LINE_LEN` = 2048 字节。单个 LogEntry 内容片段最大长度由 `LOG_ENTRY_MAX_CONTENT_LEN` 统一定义，当前为 1024 字节；超过该限制的日志行会被拆分为多个 LogEntry 片段交付，不截断、不停止泳道。首个 timestamp 之前的无时间戳启动日志暂存区大小为 `LOG_COLLECTOR_PENDING_UNTIMED_LEN`，当前默认值是 `LOG_COLLECTOR_MAX_LINE_LEN * 8`。只有该暂存区溢出时，采集器才会进入 fatal 状态，因为此时无法继续无损保存待排序内容。
...
无 timestamp 日志行不会停止采集，适合 boot banner、第三方库输出和 continuation line。此类日志使用同一 source 的当前 timestamp；如果该 timestamp 已落后于全局已观察最大 timestamp，则会被推进到全局最大 timestamp 之后。首个 timestamp 前的启动日志会等到同一 source 的首个 timestamp 后再输出，避免晚启动 source 产生倒序条目。若该 source 始终没有 timestamp，刷新阶段会使用位于全局已观察最大 timestamp 之后的 fallback timestamp 输出。
...
**文件**: `Log/LogMerger.h` / `Log/LogMerger.c`
...
**文件**: `Log/SwimLaneRenderer.h` / `Log/SwimLaneRenderer.c`
...
SystemView-only 模式只读取配置的 SystemView RTT 通道并生成 `sysview_<timestamp>.SVDat`，实际文件名同样使用上述 timestamp/pid/seq 格式，扩展名为 `.SVDat`。核心日志消费模式与 SystemView 同时启用时，`--systemview-channel` 不能与任何已启用核心日志 source 的通道复用。
...
**文件**: `RTT/RTTBridge.h` / `RTT/RTTBridge.c`
...
RTTBridge_GetLogFile()            - 获取旧式日志文件路径配置
...
| 模块           | 文件                                     | 职责                                     |
+----------------+------------------------------------------+------------------------------------------+
| LOG            | Log/Log.h, Log/Log.c                     | 主诊断日志、时间戳文件创建、ANSI 过滤和   |
|                |                                          | swimlane 文件持久化                       |
+----------------+------------------------------------------+------------------------------------------+
...
```
--addr, -a <hex_address>  RTT region 后端地址（十六进制）；0 表示使用后端基地址
--size, -s <hex_size>     用于映射、搜索和校验的 RTT region 大小（十六进制）；0 表示使用后端映射大小
--shm, --device, -d <path>  后端访问路径；MEMSHM 使用 POSIX 共享内存名称，SMEM 使用 SharedMem 设备路径
```

#### Terminal 服务参数（可选）

```
--console, -c           启用 Console 模式（使用 stdin/stdout）
...
泳道模式是严格时序视图。每条可排序业务日志应输出 `[timestamp_us] log content`，其中 `timestamp_us` 是同一硬件时间基准下的十进制微秒值。timestamp prefix 必须使用 `[]` 包裹；裸数字、PID、错误码或裸 `HH:MM:SS` 开头的日志会作为普通内容处理。无 timestamp 的日志行只适合 boot banner、第三方库输出和 continuation line；它们会使用同一 source 上一次成功解析到的 timestamp，若该 timestamp 已落后于全局已观察最大 timestamp，则推进到全局最大 timestamp 之后。首个 timestamp 到达前的无 timestamp 启动日志会暂存；若同一 source 始终没有 timestamp，刷新阶段使用 fallback timestamp 输出；超过暂存容量会进入 fatal 状态，因为此时无法继续无损保存待排序内容。
...
macOS 原生运行只支持 MEMSHM 仿真后端，不能运行 SMEM SharedMem 硬件后端，也不能使用 `Scripts/run-console.sh`、`Scripts/run-terminal.sh`、`Scripts/run-sysview.sh` 或 `Tests/Scripts/run-smem-preflight.sh` 进行真实芯片路径验证。Linux 支持 MEMSHM 仿真后端和 SMEM SharedMem 硬件后端。
...
构建系统使用本目录的独立 CMake 配置，生成可执行文件 `tracehub`。使用 `./build.sh` 时，主二进制位于 `build/<ARCH>_<TYPE>/tracehub`，并会复制一份到 `TraceHub/tracehub` 方便脚本直接调用；MEMSHM 构建还会复制仿真程序到 TraceHub 目录。构建过程不加载仓库外的 CMake 模块或环境脚本。
...
macOS 原生运行只支持 MEMSHM 仿真后端。Linux 同时支持 MEMSHM 仿真后端和 SMEM SharedMem 硬件后端；SMEM 后端使用仓库 `SharedMem/UAPI/SharedMem.h` 作为唯一 UAPI 来源，默认设备节点为 `/dev/shared_mem0`，运行脚本加载 `SharedMem.ko` 时使用 `phys_addrs` 和 `mem_sizes` 参数。`Scripts/run-*.sh` 脚本只适用于 Linux SMEM 硬件路径，并支持 `--rtt-timeout-ms` 控制 RTTCB 发现等待时间，支持 `--log-dir` 指定自动生成日志和记录文件目录。脚本发现 `SharedMem` 已加载时默认拒绝覆盖，必须显式传入 `--force-reload` 才会卸载并重新加载模块；脚本退出时只卸载本次运行加载的模块。`Scripts/run-console.sh` 和 `Scripts/run-terminal.sh` 支持 `--source linux|rtos` 与 `--channel <n>` 选择单通道 source。`BUILD_SIM_TESTS` 默认只在 MEMSHM 后端开启；模拟程序固定使用 MEMSHM，因此 SMEM 后端不会构建模拟程序，显式启用该组合会在 CMake 配置阶段报错。Linux 模拟程序负责初始化 RTTCB、写入 channel 0 文本日志、使用 SEGGER_SYSVIEW 在 channel 2 写入 A53 事件，并读取 Linux/SystemView down-buffer；RTOS 模拟程序写入 channel 1 文本日志、等待 channel 2 和 SystemView spinlock 就绪后使用 SEGGER_SYSVIEW 写入 R5 事件，并读取 RTOS down-buffer。两个模拟程序共同形成多核 SystemView 二进制 SVDat 事件流，覆盖启动同步、系统描述、任务创建/切换、中断、timer、marker 和 target printf 场景。
...
-a, --addr <addr>       RTT region 后端地址，默认 0x10040000
-s, --size <size>       RTT region 大小，默认 0x20000
-c, --channel <n>       所选服务的 RTT 通道
...
--source linux|rtos     选择单通道核心 source，默认 linux
...
```bash
bash Tests/Scripts/run-smem-preflight.sh --use-loaded-module --device /dev/shared_mem0 --log-dir ./logs
```
...
-a, --addr <addr>          RTT region 后端地址，默认 0x10040000
-s, --size <size>          RTT region 大小，默认 0x20000
-d, --device <path>        SharedMem 设备路径，默认 /dev/shared_mem0
...
--linux-channel <n>        Linux RTT up-buffer 通道，默认 0
--rtos-channel <n>         RTOS RTT up-buffer 通道，默认 1
--systemview-channel <n>   SystemView RTT up-buffer 通道，默认 2
--log-dir <dir>            生成日志目录，默认临时目录
--startup-only             只验证启动和 main log 创建
--require-data             要求 Linux/RTOS/SystemView 数据文件非空，默认启用
--linux-marker <regex>     要求 Linux 日志匹配指定正则
--rtos-marker <regex>      要求 RTOS 日志匹配指定正则
```
