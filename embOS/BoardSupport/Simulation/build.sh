...
print_configuration() {
  print_section "embOS Simulation Build Configuration"
  echo ""
  echo "Build Type:              ${BUILD_TYPE}"
  echo "Host:                    ${HOST_SYSTEM}"
  echo "Host Processor:          ${HOST_PROCESSOR}"
  echo "Target Width:            ${TARGET_BITS}-bit"
  echo "Arch Option:             ${ARCH}"
  ...
  echo ""
  echo "Build Options:"
  echo "  Clean Build:           $(cmake_bool "${CLEAN_BUILD}")"
  echo "  Verbose:               $(cmake_bool "${VERBOSE}")"
  echo "  Coverage:              $(cmake_bool "${ENABLE_COVERAGE}")"
  echo "  ASAN:                  $(cmake_bool "${ENABLE_ASAN}")"
  echo "  GUI:                   ${GUI}"
  echo "  Tick Trace:            $(cmake_bool "${TRACE_TICK}")"
  echo "  Warnings as Errors:    $(cmake_bool "${WARNINGS_AS_ERRORS}")"
  echo "  Allocator Interpose:   ${ALLOCATOR_INTERPOSITION}"
  echo ""
  echo "Build Target:"
  echo "  - ${TARGET_NAME}"
  if [[ "${APPLICATION_SOURCE}" != "" ]]; then
    echo "Application:             ${APPLICATION_SOURCE}"
  else
    echo "Application:             CMake default"
  fi
  if [[ "${LIBRARY_MODE}" != "" ]]; then
    echo "Library Mode:            ${LIBRARY_MODE}"
  else
    echo "Library Mode:            CMake default"
  fi
  echo ""
}
...
build_project() {
  ...
  print_section "Building embOS Simulation"
...
print_build_summary() {
  ...
  echo ""
  echo "Build Directory:   ${BUILD_DIR}"
  echo "Output Root:       ${SCRIPT_DIR}/Output"
  echo "Target Width:      ${TARGET_BITS}-bit"
  echo "Build Type:        ${BUILD_TYPE}"
  echo ""
}
