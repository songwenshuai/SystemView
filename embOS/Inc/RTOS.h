...
#define OS_INT_EnterIntStack()                     // Int stack is automatically used in simulation
#define OS_INT_LeaveIntStack()                     // Int stack is automatically used in simulation
...
#define OS_IDLE()                            return &OS_Global
#define OS_I64                               long long
#define OS_U32                               unsigned OS_I32
#define OS_TASK_EnterRegion                  OS_SIM_EnterRegionFunc
...
#if (OS_64BIT == 1)
  #define OS_I32        int
  #define OS_STACK_ADR  unsigned OS_I64
#else
  //
  // Use long instead of int for 32-bit integers on a 64-bit system to be compatible to I32 and U32 from Global.h.
  //
  #define OS_I32        long
#endif

//
// Set terminated tasks into task state "suspended". This is required,
// because the Windows scheduler may not suspend a calling thread when
// we call switch. If this happens, a task which is not in the "ready"
// state will continue to run and cause embOS to crash. To avoid such
// situation, we do not start any task which is not in ready state.
// To detect terminated tasks, which normally do not have a special
// task state, we set terminated tasks into suspended state.
//
#define OS_SUSPEND_TASK_ON_TERMINATE (1)

//
// TCB extension for CPU port
//
#define OS_TCB_CPU_EX \
  void* pfRoutine;    \
  void* pThreadInfo;  \
  int   DICnt;        \
  int   RegionCnt;    \
...
*/
typedef struct {
  int Dummy;  // Not used in simulation ... Stack is managed by windows
} OS_REGS;
...
typedef int  OS_ISR_CONTROLLER(void);

void  OS_SIM_EnterISR         (void);
void  OS_SIM_LeaveISR         (void);
void* OS_SIM_CreateISRThread  (OS_ISR_HANDLER* pfISR, OS_ISR_CONTROLLER* pfController);
void* OS_SIM_CreateISRThreadEx(OS_ISR_HANDLER* pfISR, OS_ISR_CONTROLLER* pfController, const char* sThreadName);
void  OS_SIM_DeleteISRThread  (void* hThreadHandle);
void  OS_SIM_EnterSysCall     (void);
void  OS_SIM_LeaveSysCall     (void);
void  OS_SIM_EnterRegionFunc  (void);
...
#define OS_INT_Disable()     OS__di()
#define OS_INT_DisableAll()  OS__di()
#define OS_INT_Enable()      OS__ei()
#define OS_INT_EnableAll()   OS__ei()

/*********************************************************************
*
*       embOS interrupt preserve/restore support
*/
OS_U32 OS_GetIntState(void);
void   OS_SetIntState(OS_U32 Int);

#define OS_INT_Preserve(p)     (*(p) = (OS_U32)OS_GetIntState())
#define OS_INT_PreserveAll(p)  (*(p) = (OS_U32)OS_GetIntState())
#define OS_INT_Restore(p)      OS_SetIntState(*(p))
#define OS_INT_RestoreAll(p)   OS_SetIntState(*(p))
...
*/
#if (OS_64BIT == 1)
  #define OS_Init  OS_Init_x86_64
#else
  #define OS_Init  OS_Init_x86
#endif

/*********************************************************************
*
*       System specific implementation for thread safety
*/
void OS_ThreadSafe_Lock  (void);
void OS_ThreadSafe_Unlock(void);
...
#define OS_VERSION_GENERIC  52200u
#define OS_VERSION          (OS_VERSION_GENERIC + (OS_PORT_REVISION * 25u))

/*********************************************************************
*
*  SEGGER embOS pre-defined symbol which can be useful for applications
*  which are built with and w/o embOS.
*
**********************************************************************
*/
#define SEGGER_EMBOS  (1)
...
*     generic sources for a specific core and compiler combination.
...
*
*  Compile time switches which may be changed by the user to
...
*
*  If defines are not set by the user default values are used.
*
**********************************************************************
*/
//
// Enables runtime debug checks
//
#ifndef   OS_DEBUG
...
//
// Percentage of stack usage that will be detected as a stack overflow error
//
...
//
// Support for embOSView tick step
//
...
//
// Support for generic API trace tools like e.g. SystemView
//
#ifndef   OS_SUPPORT_TRACE_API
  #define OS_SUPPORT_TRACE_API              OS_SUPPORT_TRACE_API_DEFAULT
#endif

//
// Generates additional Trace API-End events
//
...
//
// Support for task and OS object names
//
...
//
// Support for task context extensions
//
...
//
// Support for task statistic information
//
...
//
// Explicitly initialization of internal embOS variables
//
...
//
// Support for embOS software timers
//
...
//
// Support for peripheral power control
//
...
//
// Number of peripherals which can be used
//
...
//
// Number of cores that should access a spinlock
//
...
*/
//
// Support for thread safe heap functions
//
...
//
#ifndef   OS_STACK_GROWS_TOWARD_HIGHER_ADDR
...
#endif
//
// Per default OS_QUEUE_Create() aligns the queue buffer address and size to int.
...
/*********************************************************************
*
*       Optimizations for pointer and 16/32/64-bit operations.
*
*  On a specific CPU some operations might not be atomic.
*  If necessary, specific operations can be made atomic by setting the
*  according define to 0 which disables interrupts or task switches
*  while executing the operation.
*
*  For example on a 16-bit CPU 32-bit operations might not be atomic.
*  If a 32-bit value is read in a task with two 16-bit accesses an
*  interrupt could change the value which results in a wrong value used
*  by the task.
*
*  The default of OS_U64_OP_IS_ATOMIC is set to 0 since we mainly use
*  CPUs where 64-bit operations are not atomic.
*/
#ifndef   OS_PTR_OP_IS_ATOMIC
  #define OS_PTR_OP_IS_ATOMIC  (1)
#endif

#ifndef   OS_INT_OP_IS_ATOMIC
...
*       OS_Idle()
*
*  This implementation works for most cores expect those which run
*  OS_ChangeTask() from an exception which needs to be ended before
*  OS_Idle() can be executed.
*
**********************************************************************
*/
#ifndef OS_IDLE
  #if (OS_DEBUG != 0)
    //
    // We should never arrive here, since the Idle loop should not return
    //
    #define OS_IDLE() { OS_Idle();                     \
                        OS_Error(OS_ERR_IDLE_RETURNS); \
                      }
  #else
    #define OS_IDLE() { OS_Idle(); }
  #endif
...
*  for these functions where necessary.
*
**********************************************************************
*/
#ifndef   OS_MEMSET
...
#ifndef OS_TASK_PRIO
  #if (OS_SIZEOF_INT >= 4u)
    #define OS_TASK_PRIO      OS_U32
  #else
    #define OS_TASK_PRIO      OS_U8
  #endif
#endif
...
// Port 100..109 *********************************
  ...
  OS_ERR_MB_BUFFER_SIZE                  = (106u),  // For 16-bit CPUs, the maximum buffer size for a mailbox (65,535 bytes) exceeded.

// OS generic ************************************
  OS_ERR_TASK_STACK_SIZE_INSUFFICIENT    = (115u),  // Task stack size too small.
  ...
  OS_ERR_TASK_STACK_OVERFLOW             = (120u),  // Task stack overflow.
...
// System/interrupt stack
  OS_ERR_SYSTEM_STACK_OVERFLOW           = (125u),  // System stack overflow.
  OS_ERR_INTERRUPT_STACK_OVERFLOW        = (126u),  // Interrupt stack overflow.
...
// invalid or non-initialized data structures
  ...
  OS_ERR_MAILBOX_NOT1                    = (135u),  // One of the following 1-byte mailbox functions has been used on a multi-byte mailbox: OS_MAILBOX_Get1(), OS_MAILBOX_GetBlocked1(), OS_MAILBOX_GetTimed1(), OS_MAILBOX_Put1(), OS_MAILBOX_PutBlocked1(), OS_MAILBOX_PutFront1(), OS_MAILBOX_PutFrontBlocked1() or OS_MAILBOX_PutTimed1().
...
// Mailbox errors
  OS_ERR_MAILBOX_INUSE                   = (147u),  // Mailbox in use.
  OS_ERR_MAILBOX_NOT_INUSE               = (148u),  // Mailbox not in use.
...
// Not matching routine calls or macro usage
  ...
  OS_ERR_LEAVEINT                        = (152u),  // OS_INT_Leave()/OS_INT_LeaveNestable() called without OS_INT_Enter()/OS_INT_EnterNestable().
...
// Communication errors
  OS_ERR_NESTED_RX_INT                   = (180u),  // OS_COM_OnRx() interrupt handler for embOSView is nested. Disable nestable interrupts.
...
// Event object
  ...
  OS_ERR_EVENTOBJ_RESETMODE              = (213u),  // OS_MULTIOBJ_IsEventSignaled() used with reset mode other than OS_EVENT_RESET_MODE_MANUAL.
...
// Readers-Writer Lock error
  OS_ERR_RWLOCK_INVALID                  = (257u),  // Readers-Writer Lock control block invalid, not initialized or overwritten.
  OS_ERR_2USE_RWLOCK                     = (258u),  // Readers-Writer Lock has been initialized by calling a create function twice.
...
// MPU sanity check buffer not set
  OS_ERR_MPU_SANITY_BUFFER_NOT_SET       = (263u),  // MPU sanity check buffer not set with OS_MPU_SetSanityCheckBuffer().

// MPU invalid index used
  OS_ERR_MPU_INVALID_ATTR_INDEX          = (264u),  // MPU Invalid index used with OS_ARMv8M_SetMPUAttribute().

// MPU sanity check buffer not set
  OS_ERR_MULTIOBJ_INVALID_ROUTINE        = (265u),  // Invalid condition routine used with OS_MULTIOBJ_Wait(), OS_MULTIOBJ_WaitBlocked() or OS_MULTIOBJ_WaitTimed().

// Invalid API called from a device driver
  OS_ERR_ILLEGAL_IN_DEVICE_DRIVER        = (266u)   // Invalid API called from a device driver
} OS_STATUS;
...
#define OS_ASSERT_OS_INIT_CALLED()  OS_ASSERT((OS_Global.InitCalled != 0u),      OS_ERR_INIT_NOT_CALLED)
...
#define OS_TRACE_ID_TIME_CVT_MSEC_TO_CYCLES          (310u)
...
#define OS_TRACE_ID_MULTIOBJ_WAIT                    (323u)
#define OS_TRACE_ID_MULTIOBJ_WAIT_BLOCKED            (324u)
#define OS_TRACE_ID_MULTIOBJ_WAIT_TIMED              (325u)
#define OS_TRACE_ID_QUEUE_HASFREESPACE               (326u)
...
/*********************************************************************
*
*       typedef definitions for structures
*
**********************************************************************
*/
...
typedef struct OS_MULTIWAIT_LIST_STRUCT OS_MULTIWAIT_LIST;
typedef struct OS_WD_STRUCT             OS_WD;

/*********************************************************************
*
*       typedef definitions for function pointer
*
**********************************************************************
*/
typedef void    OS_ROUTINE_VOID         (void);
typedef void    OS_ROUTINE_VOID_PTR     (void* p);
typedef OS_BOOL OS_ROUTINE_BOOL_VOID_PTR(OS_CONST_PTR void* pObject, void* pParam);
typedef void    OS_ROUTINE_CHAR         (OS_U8 Data);
typedef void    OS_ROUTINE_WD_PTR       (OS_CONST_PTR OS_WD* pWD);
typedef void    OS_ROUTINE_TASK_PTR     (OS_CONST_PTR OS_TASK* pTask);
typedef void    OS_ROUTINE_OSGLOBAL_PTR (OS_GLOBAL* pGlobal);

/*********************************************************************
*
*       typedef definitions for OS_WAIT_OBJ and OS_WAIT_LIST
*/
...
struct OS_WAIT_LIST_STRUCT {
  ...
  OS_TASK*           pTask;
  OS_MULTIWAIT_LIST* pMultiWaitList;
};
...
//
#ifndef   OS_MPU_MAX_REGIONS
  #define OS_MPU_MAX_REGIONS  (16u)
#endif

#ifndef   OS_MPU_SUPERVISOR_CALL
  #define OS_MPU_SUPERVISOR_CALL()
#endif


//
// Bits 0..7   encode the generic data access permissions.
// Bits 8..15  encode the core specific MPU data access permissions (defined in OSCHIP.h).
// Bit  16     encodes the code execution permissions
//

typedef enum {
  OS_MPU_INVALID,        // 0x0000
  OS_MPU_NOACCESS,       // 0x0001
  OS_MPU_READONLY,       // 0x0002
  OS_MPU_READWRITE       // 0x0003
} OS_MPU_DATA_PERMISSION;

typedef enum {
  OS_MPU_EXECUTION_ALLOWED,              // 0x0000 0000
  OS_MPU_EXECUTION_DISALLOWED = 0x10000  // 0x0001 0000
} OS_MPU_CODE_PERMISSION;

#define OS_MPU_DATA_PERMISSION_MASK          (0x0000FFFFu)
#define OS_MPU_GENERIC_DATA_PERMISSION_MASK  (0x000000FFu)
#define OS_MPU_CORE_PERMISSION_MASK          (0x0000FF00u)
#define OS_MPU_CODE_PERMISSION_MASK          (0x00010000u)
#define OS_MPU_CODE_PERMISSION_SHIFT         (16u)
#define OS_MPU_CORE_PERMISSION_SHIFT         (8u)

#ifndef   OS_MPU_CORE_FULL_ACCESS
  #define OS_MPU_CORE_FULL_ACCESS  (0u)
#endif
...
typedef enum {
  ...
  OS_MPU_ERROR_SVC,
  OS_MPU_ERROR_INVALID_DEVICE_DRIVER,
  OS_MPU_ERROR_INVALID_CONDITION_ROUTINE
} OS_MPU_ERRORCODE;
...
typedef struct {
  const OS_MPU_OBJ*    pAllowedObjects;              // Array of objects to which the task has access
  OS_MPU_REGION        Regions[OS_MPU_MAX_REGIONS];  // Regions to have the task access
  OS_MPU_THREAD_STATE  ThreadState;                  // Configured task privileged state
#ifdef OS_LIBMODE_SAFE
  void*                pBuffer;                      // Pointer to a buffer which holds a copy of the MPU hardware register for sanity check
#endif
  OS_BOOL              WriteMPURegs;                 // Flag to signal to program MPU hardware register again instead of restore saved MPU hardware register from the task stack
  OS_BOOL              InDeviceDriver;               // Defines whether a device driver gets currently executed
} OS_MPU;
...
typedef struct OS_MPU_API_LIST_STRUCT {
  ...
  OS_MPU_THREAD_STATE (*pfGetPrivilegedState)  (void);
...
typedef void (*OS_MPU_DEVICE_DRIVER_FUNC) (void* p);  // Obsolete but kept for compatibility
typedef void OS_ROUTINE_TASK_PTR_ERRORCODE(OS_CONST_PTR OS_TASK* pTask, OS_MPU_ERRORCODE ErrorCode);
...
#define OS_MPU_ExtendContext  OS_MPU_ContextExtension
...
#ifndef   OS_MPU_GET_PRIVSTATE
  #define OS_MPU_GET_PRIVSTATE  OS_MPU_GetPrivilegedState
#endif

#ifndef   OS_MPU_ASSERT_PRIVSTATE
  #define OS_MPU_ASSERT_PRIVSTATE()  { OS_MPU_THREAD_STATE ThreadState;                                          \
                                                                                                                 \
                                       ThreadState = OS_MPU_GET_PRIVSTATE();                                     \
                                       if (ThreadState == OS_MPU_THREAD_STATE_UNPRIVILEGED) {                    \
                                         OS_MPU_SUPERVISOR_CALL();                                               \
                                         OS_MPU_TerminateTask(OS_Global.pCurrentTask, OS_MPU_ERROR_INVALID_API); \
                                       }                                                                         \
                                     }
#endif

#ifndef   OS_MPU_PRIVSTATE_ENTER
  #define OS_MPU_PRIVSTATE_ENTER()   { OS_MPU_THREAD_STATE ThreadState;                       \
                                                                                              \
                                       ThreadState = OS_MPU_GET_PRIVSTATE();                  \
                                       if (ThreadState == OS_MPU_THREAD_STATE_UNPRIVILEGED) { \
                                         OS_MPU_SUPERVISOR_CALL();                            \
  ...
  #define OS_MPU_PRIVSTATE_LEAVE()     OS_MPU_LeavePrivilegedState(ThreadState); \
                                     }
#endif
...
*/
//
// A TCB contains a pointer to a linked list of type OS_MULTIWAIT_LIST.
// The task needs to know what RTOS objects it is waiting for.
// Therefore, pWaitlist points to the according wait list.
//
struct OS_MULTIWAIT_LIST_STRUCT {
  OS_MULTIWAIT_LIST*        pNext;
  OS_MULTIWAIT_LIST*        pPrev;
  OS_WAIT_LIST*             pWaitList;
  OS_ROUTINE_BOOL_VOID_PTR* pfRoutine;
  void*                     pParameter;
};

//
// Used for memory provided by the application that holds the wait list and multi wait list entries
//
typedef struct {
  OS_WAIT_LIST      WaitList;
  OS_MULTIWAIT_LIST MultiWaitList;
} OS_MULTIOBJ;

//
// Information about the RTOS object, the completion function and the optional parameter
//
typedef struct {
  void*                     pObject;
  OS_ROUTINE_BOOL_VOID_PTR* pfRoutine;
...
struct OS_TASK_STRUCT {
  OS_TASK*                    pNext;            // Points to the TCB of the next task in the list (with equal or lower priority). Needs to be first element since functions like OS_UnlinkTask() presume it for more efficient handling.
  OS_REGS OS_STACKPTR*        pStack;           // Contains the task stack pointer. Needs to be second element since it used by the assembly part.
  OS_MULTIWAIT_LIST*          pMultiWaitList;   // Points to a list of waitable object lists if task is suspended
  OS_TIME                     Timeout;          // Timeout used with timed API functions like OS_TASK_Delay()
  OS_TASK_PRIO                Priority;         // Task priority, could be raised during Mutex handling
  OS_TASK_PRIO                BasePrio;         // Initial task priority
  volatile OS_TASKEVENT       Events;           // Event storage
  OS_TASKEVENT                EventMask;        // Event mask
#if (OS_SUPPORT_RR != 0)
  OS_TASK*                    pPrev;            // Pointer to previous TCB in the last list, required only in builds with doubly-linked task lists (Round-robin)
#endif
#if (OS_SUPPORT_TRACKNAME != 0)
  OS_ROM_DATA const char*     sName;            // Pointer to task name
#endif
  ...
  OS_U8 OS_STACKPTR*          pStackBase;       // First byte of stack. Not required for functionality, just for analysis
#endif
  ...
  OS_EXTEND_TASK_CONTEXT_LINK ContextExtension; // Task context extension
#endif
#if ((OS_SUPPORT_TLS != 0) && (OS_SUPPORT_SAVE_RESTORE_HOOK != 0))
   OS_TLS_PTR                 pTLS;             // Optional thread local storage pointer, can only be used when task context extensions are available
#endif
#if (OS_SUPPORT_MPU != 0)
  OS_MPU                      MPU_Config;       // Optional embOS MPU
#endif
...
#endif
#ifdef OS_TCB_CPU_EX
  OS_TCB_CPU_EX                                 // Allow port specific extension to the task control block. Used e.g. in embOS 8051 IAR and embOS simulation.
#endif
};
...
typedef struct {
  ...
  OS_U32       IntFreq;                        // typical 1000 for 1 KHz system tick
...
//
...
#if (OS_SUPPORT_TIMER != 0)
  OS_TIMER*                    pTimer;                        // Linked list of all active software timers
  ...
  OS_MAILBOX*                  pMailboxRoot;                  // Root pointer to linked list of all mailboxes
  OS_SEMAPHORE*                pSemaRoot;                     // Root pointer to linked list of all semaphores
  OS_EVENT*                    pEventRoot;                    // Root pointer to linked list of all event objects
  OS_RWLOCK*                   pRWLockRoot;                   // Root pointer to linked list of all readers-writer locks
#endif
#if (OS_SUPPORT_TRACKNAME != 0)
  OS_OBJNAME*                  pObjNameRoot;                  // Root pointer to linked list of all named objects
#endif
  OS_ROUTINE_VOID*             pfTaskStartHook;               // Pointer to a routine which is executed at task start
#if (OS_SUPPORT_SAVE_RESTORE_HOOK != 0)
  OS_EXTEND_TASK_CONTEXT*      pDefaultTaskContextExtension;  // Pointer to the default task context extension
#endif
#if (OS_SUPPORT_PROFILE != 0)
  OS_U32                       ExecStart;                     // Time stamp for profiling
#endif
...
#if (OS_SUPPORT_TICKSTEP != 0)
  volatile OS_U8               TickStep;                      // Used for tick step feature used by embOSView
#endif
#if (OS_SUPPORT_MPU != 0)
  OS_MPU_MEM_CONFIG            MemConfig;                     // RAM/ROM memory settings for embOS MPU
#endif
};
...
*
*  Field offsets can be used by remote debugger for thread-awareness,
...
typedef struct OS_DEBUG_INFO_STRUCT
{
  ...
  OS_U8 OS_TASK_pStackBase_Offs;       // Offset of pStackBase
...
//
// Handle Definition (storage is actually allocated) versus Declaration
...
//
#if (OS_SUPPORT_INT_PRIORITY == 0)
OS_EXTERN OS_SADDR OS_GLOBAL            OS_Global            OS_BSS_SECTION_ATTRIBUTE(OS_Global);             //lint !e9075 MISRA C:2012 Rule 8.4, required
#else
...
OS_EXTERN          volatile OS_UINT     OS_CPU_Load          OS_BSS_SECTION_ATTRIBUTE(OS_CPU_Load);           //lint !e9075 MISRA C:2012 Rule 8.4, required
#ifdef OS_U64
OS_EXTERN          volatile OS_I64      OS_IdleCnt           OS_BSS_SECTION_ATTRIBUTE(OS_IdleCnt);            //lint !e9075 MISRA C:2012 Rule 8.4, required
#else
...
OS_BOOL  OS_IsRunning (void)                                               OS_TEXT_SECTION_ATTRIBUTE(OS_IsRunning);
void     OS__Start    (void)                                               OS_TEXT_SECTION_ATTRIBUTE(OS__Start);
void     OS_Stop      (void)                                               OS_TEXT_SECTION_ATTRIBUTE(OS_Stop);

//
// This is a speed optimized version of OS_IsRunning() without debug checks and API trace.
// It is designed to be used  by SEGGER software like e.g. J-Link memory communication (JLINKMEM_Process.c) only.
//
#define OS__IsRunning()  OS_Global.IsRunning

#if (OS_DEBUG != 0)
  #define OS_Start()  { OS_UINT Version;                                            \
                        Version = OS_INFO_GetVersion();                             \
                        OS_ASSERT((Version == OS_VERSION), OS_ERR_VERSION_MISMATCH) \
                        OS__Start();                                                \
                      }
#else
  #define OS_Start()  OS__Start()
#endif
...
OS_TASK_PRIO            OS_TASK_GetPriority               (OS_CONST_PTR OS_TASK* pTask)                                                                                                                                                          OS_TEXT_SECTION_ATTRIBUTE(OS_TASK_GetPriority);
...
void                    OS_TASK_SetPriority               (OS_TASK* pTask, OS_TASK_PRIO Priority)                                                                                                                                                OS_TEXT_SECTION_ATTRIBUTE(OS_TASK_SetPriority);
...
int     OS_SEMAPHORE_GetValue   (OS_CONST_PTR OS_SEMAPHORE* pSema)       OS_TEXT_SECTION_ATTRIBUTE(OS_SEMAPHORE_GetValue);
void    OS_SEMAPHORE_Give       (OS_SEMAPHORE* pSema)                    OS_TEXT_SECTION_ATTRIBUTE(OS_SEMAPHORE_Give);
void    OS_SEMAPHORE_GiveMax    (OS_SEMAPHORE* pSema, OS_UINT MaxValue)  OS_TEXT_SECTION_ATTRIBUTE(OS_SEMAPHORE_GiveMax);
...
**********************************************************************
*/
void    OS_MAILBOX_Clear           (OS_MAILBOX* pMailbox)                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Clear);
void    OS_MAILBOX_Create          (OS_MAILBOX* pMailbox, OS_U16 sizeofMsg, OS_UINT maxnofMsg, void* pBuffer) OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Create);
void    OS_MAILBOX_Delete          (OS_MAILBOX* pMailbox)                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Delete);
char    OS_MAILBOX_Get             (OS_MAILBOX* pMailbox, void* pMessage)                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Get);
char    OS_MAILBOX_Get1            (OS_MAILBOX* pMailbox, char* pMessage)                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Get1);
void    OS_MAILBOX_GetBlocked      (OS_MAILBOX* pMailbox, void* pMessage)                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetBlocked);
void    OS_MAILBOX_GetBlocked1     (OS_MAILBOX* pMailbox, char* pMessage)                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetBlocked1);
OS_UINT OS_MAILBOX_GetMessageCnt   (OS_CONST_PTR OS_MAILBOX* pMailbox)                                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetMessageCnt);
char    OS_MAILBOX_GetTimed        (OS_MAILBOX* pMailbox, void* pMessage, OS_TIME Timeout)                    OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetTimed);
char    OS_MAILBOX_GetTimed1       (OS_MAILBOX* pMailbox, char* pMessage, OS_TIME Timeout)                    OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetTimed1);
char    OS_MAILBOX_GetPtr          (OS_MAILBOX* pMailbox, void** ppMessage)                                   OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetPtr);
void    OS_MAILBOX_GetPtrBlocked   (OS_MAILBOX* pMailbox, void** ppMessage)                                   OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_GetPtrBlocked);
OS_BOOL OS_MAILBOX_IsInUse         (OS_CONST_PTR OS_MAILBOX* pMailbox)                                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_IsInUse);
char    OS_MAILBOX_Peek            (OS_CONST_PTR OS_MAILBOX* pMailbox, void* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Peek);
void    OS_MAILBOX_Purge           (OS_MAILBOX* pMailbox)                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Purge);
char    OS_MAILBOX_Put             (OS_MAILBOX* pMailbox, OS_CONST_PTR void* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Put);
char    OS_MAILBOX_Put1            (OS_MAILBOX* pMailbox, OS_CONST_PTR char* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Put1);
void    OS_MAILBOX_PutBlocked      (OS_MAILBOX* pMailbox, OS_CONST_PTR void* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutBlocked);
void    OS_MAILBOX_PutBlocked1     (OS_MAILBOX* pMailbox, OS_CONST_PTR char* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutBlocked1);
char    OS_MAILBOX_PutFront        (OS_MAILBOX* pMailbox, OS_CONST_PTR void* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutFront);
char    OS_MAILBOX_PutFront1       (OS_MAILBOX* pMailbox, OS_CONST_PTR char* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutFront1);
void    OS_MAILBOX_PutFrontBlocked (OS_MAILBOX* pMailbox, OS_CONST_PTR void* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutFrontBlocked);
void    OS_MAILBOX_PutFrontBlocked1(OS_MAILBOX* pMailbox, OS_CONST_PTR char* pMessage)                        OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutFrontBlocked1);
OS_BOOL OS_MAILBOX_PutTimed        (OS_MAILBOX* pMailbox, OS_CONST_PTR void* pMessage, OS_TIME Timeout)       OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutTimed);
OS_BOOL OS_MAILBOX_PutTimed1       (OS_MAILBOX* pMailbox, OS_CONST_PTR char* pMessage, OS_TIME Timeout)       OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutTimed1);
void    OS_MAILBOX_WaitBlocked     (OS_MAILBOX* pMailbox)                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_WaitBlocked);
char    OS_MAILBOX_WaitTimed       (OS_MAILBOX* pMailbox, OS_TIME Timeout)                                    OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_WaitTimed);
...
void    OS_QUEUE_Create        (OS_QUEUE* pQ, void* pBuffer, OS_UINT Size)                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_Create);
...
int     OS_QUEUE_GetPtr        (OS_QUEUE* pQ, void** ppMessage)                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_GetPtr);
int     OS_QUEUE_GetPtrBlocked (OS_QUEUE* pQ, void** ppMessage)                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_GetPtrBlocked);
int     OS_QUEUE_GetPtrTimed   (OS_QUEUE* pQ, void** ppMessage, OS_TIME Timeout)                                                 OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_GetPtrTimed);
OS_BOOL OS_QUEUE_HasFreeSpace  (OS_CONST_PTR OS_QUEUE* pQ, OS_UINT Size)                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_HasFreeSpace);
OS_BOOL OS_QUEUE_IsInUse       (OS_CONST_PTR OS_QUEUE* pQ)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_IsInUse);
int     OS_QUEUE_PeekPtr       (OS_CONST_PTR OS_QUEUE* pQ, void** ppMessage)                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PeekPtr);
void    OS_QUEUE_Purge         (OS_QUEUE* pQ)                                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_Purge);
int     OS_QUEUE_Put           (OS_QUEUE* pQ, OS_CONST_PTR void* pMessage, OS_UINT Size)                                         OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_Put);
int     OS_QUEUE_PutEx         (OS_QUEUE* pQ, OS_CONST_PTR OS_QUEUE_SRCLIST* pMessageList, OS_UINT NumMessages)                  OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PutEx);
void    OS_QUEUE_PutBlocked    (OS_QUEUE* pQ, OS_CONST_PTR void* pMessage, OS_UINT Size)                                         OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PutBlocked);
void    OS_QUEUE_PutBlockedEx  (OS_QUEUE* pQ, OS_CONST_PTR OS_QUEUE_SRCLIST* pMessageList, OS_UINT NumMessages)                  OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PutBlockedEx);
char    OS_QUEUE_PutTimed      (OS_QUEUE* pQ, OS_CONST_PTR void* pMessage, OS_UINT Size, OS_TIME Timeout)                        OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PutTimed);
char    OS_QUEUE_PutTimedEx    (OS_QUEUE* pQ, OS_CONST_PTR OS_QUEUE_SRCLIST* pMessageList, OS_UINT NumMessages, OS_TIME Timeout) OS_TEXT_SECTION_ATTRIBUTE(OS_QUEUE_PutTimedEx);
...
**********************************************************************
*/
OS_INT OS_MULTIOBJ_Wait       (                        OS_CONST_PTR OS_MULTIOBJ_COND* pMultiObjCond, OS_UINT NumObjects)                  OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_Wait);
OS_INT OS_MULTIOBJ_WaitBlocked(OS_MULTIOBJ* pMultiObj, OS_CONST_PTR OS_MULTIOBJ_COND* pMultiObjCond, OS_UINT NumObjects)                  OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_WaitBlocked);
OS_INT OS_MULTIOBJ_WaitTimed  (OS_MULTIOBJ* pMultiObj, OS_CONST_PTR OS_MULTIOBJ_COND* pMultiObjCond, OS_UINT NumObjects, OS_TIME Timeout) OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_WaitTimed);

//
// Pre-defined Completion functions
//
OS_BOOL OS_MULTIOBJ_IsEventSignaled   (OS_CONST_PTR void* pEvent,     void* pMask)        OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsEventSignaled);
OS_BOOL OS_MULTIOBJ_IsTokenInSema     (OS_CONST_PTR void* pSemaphore, void* pNumTokens)   OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsTokenInSema);
OS_BOOL OS_MULTIOBJ_IsMessageInMailbox(OS_CONST_PTR void* pMailbox,   void* pNumMessages) OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsMessageInMailbox);
OS_BOOL OS_MULTIOBJ_IsSpaceInMailbox  (OS_CONST_PTR void* pMailbox,   void* pNumMessages) OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsSpaceInMailbox);
OS_BOOL OS_MULTIOBJ_IsMessageInQueue  (OS_CONST_PTR void* pQueue,     void* pNumMessages) OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsMessageInQueue);
OS_BOOL OS_MULTIOBJ_IsSpaceInQueue    (OS_CONST_PTR void* pQueue,     void* pSize)        OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsSpaceInQueue);
OS_BOOL OS_MULTIOBJ_IsBlockInMemPool  (OS_CONST_PTR void* pMEMF,      void* pNumBlocks)   OS_TEXT_SECTION_ATTRIBUTE(OS_MULTIOBJ_IsBlockInMemPool);
...
void OS_WD_Trigger(OS_WD* pWD)                                             OS_TEXT_SECTION_ATTRIBUTE(OS_WD_Trigger);
...
//
  ...
  OS_INTERWORK void OS_INT_DisableFunc(void)              OS_TEXT_SECTION_ATTRIBUTE(OS_INT_DisableFunc);
  #define OS_INT_Disable()       OS_INT_DisableFunc()
#endif
  ...
  OS_INTERWORK void OS_INT_EnableFunc(void)               OS_TEXT_SECTION_ATTRIBUTE(OS_INT_EnableFunc);
  #define OS_INT_Enable()        OS_INT_EnableFunc()
#endif
  ...
  OS_INTERWORK void OS_INT_PreserveAllFunc(OS_U32 *p)     OS_TEXT_SECTION_ATTRIBUTE(OS_INT_PreserveAllFunc);
  ...
  OS_INTERWORK void OS_INT_RestoreAllFunc(OS_U32 *p)      OS_TEXT_SECTION_ATTRIBUTE(OS_INT_RestoreAllFunc);
...
//
// Used in embOS Sim only
//
#ifndef   OS_INT_ENTER_CALLBACK
  #define OS_INT_ENTER_CALLBACK()
#endif

#if (OS_SUPPORT_CALL_ISR != 0)  // Not allowed for some CPUs
void OS_INT_Call        (void (*pfRoutine)(void)) OS_TEXT_SECTION_ATTRIBUTE(OS_INT_Call);
void OS_INT_CallNestable(void (*pfRoutine)(void)) OS_TEXT_SECTION_ATTRIBUTE(OS_INT_CallNestable);
#endif
  ...
  // OS_INT_Enter()/OS_INT_Leave()/OS_INT_EnterNestable()/OS_INT_LeaveNestable() are not defined.
  // OS_INT_Call()/OS_INT_CallNestable() have to be used instead.
  //
#else
  ...
  //
  // Disable low priority interrupts if necessary: On CPUs, that keep interrupts enabled on ISR entry. We disable all further low priority interrupts.
  //
  ...
  //
  // Enable high priority interrupts if necessary: Some CPUs (such as M16C) disable all interrupts on ISR entry. We need to enable high priority interrupts (e.g. using the global interrupt enable flag).
  //
  ...
  //
  // Required for CPUs which do not restore DI-flag by return from interrupt instruction. Used for example in embOS Cortex-M.
  //
  #ifndef   OS_EI_ON_LEAVE
    #define OS_EI_ON_LEAVE()
  #endif
  ...
  #define OS_INT_Enter() {                                                       \
    OS_ASSERT_OS_INIT_CALLED();                                                  \
    OS_INT_ENTER_CALLBACK();                                                     \
    OS_MARK_IN_ISR();                                                            \
...
//
  ...
  #if (OS_SUPPORT_MPU == 1)
    //
    // With embOS-MPU we need to use the routine because OS_MPU_ASSERT_PRIVSTATE()
    // must be called from within the RTOS text section which is not the case when
    // OS_TASK_EnterRegion() is a macro.
    //
    #define OS_TASK_EnterRegion  OS_EnterRegionFunc
  #else
  ...
  #endif  // (OS_SUPPORT_MPU == 1)
#endif  // OS_TASK_EnterRegion
...
void    OS_POWER_UsageInc(OS_UINT Index) OS_TEXT_SECTION_ATTRIBUTE(OS_POWER_UsageInc);
void    OS_POWER_UsageDec(OS_UINT Index) OS_TEXT_SECTION_ATTRIBUTE(OS_POWER_UsageDec);
OS_UINT OS_POWER_GetMask (void)          OS_TEXT_SECTION_ATTRIBUTE(OS_POWER_GetMask);
#endif
...
void* OS_HEAP_realloc(void* pMemBlock, unsigned int Size) OS_TEXT_SECTION_ATTRIBUTE(OS_HEAP_realloc);
#endif
...
/*********************************************************************
*
*       API trace support (e.g. SystemView)
*
**********************************************************************
*/
...
*/
#if (OS_SUPPORT_PROFILE != 0)
  void   OS_EnableProfiling         (void)                        OS_TEXT_SECTION_ATTRIBUTE(OS_EnableProfiling);
...
OS_ROUTINE_CHAR* OS_COM_SetRxCallback (OS_ROUTINE_CHAR* pfRXCallback)                            OS_TEXT_SECTION_ATTRIBUTE(OS_COM_SetRxCallback);
#endif
...
void                OS_MPU_CallDeviceDriver     (OS_U32 Index, void* pParam)                                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_MPU_CallDeviceDriver);
void                OS_MPU_CallDeviceDriverEx   (OS_ROUTINE_VOID_PTR* pfRoutine, void* pParam)                                                                 OS_TEXT_SECTION_ATTRIBUTE(OS_MPU_CallDeviceDriverEx);
...
void                OS_MPU_LeavePrivilegedState (OS_MPU_THREAD_STATE State)                                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_MPU_LeavePrivilegedState);
...
/*********************************************************************
*
*       Board support package (BSP) related routines
*       (usually located in RTOSInit.c)
*
**********************************************************************
*/
void   OS_InitHW          (void);
...
**********************************************************************
*/
OS_ROM_DATA const char* OS_INFO_GetCPU      (void) OS_TEXT_SECTION_ATTRIBUTE(OS_INFO_GetCPU);
OS_ROM_DATA const char* OS_INFO_GetLibMode  (void) OS_TEXT_SECTION_ATTRIBUTE(OS_INFO_GetLibMode);
OS_ROM_DATA const char* OS_INFO_GetModel    (void) OS_TEXT_SECTION_ATTRIBUTE(OS_INFO_GetModel);
OS_ROM_DATA const char* OS_INFO_GetLibName  (void) OS_TEXT_SECTION_ATTRIBUTE(OS_INFO_GetLibName);
OS_U32                  OS_INFO_GetTimerFreq(void) OS_TEXT_SECTION_ATTRIBUTE(OS_INFO_GetTimerFreq);
...
*/
//
// Compatibility with manual (Misspelled names in older manual versions)
//
...
//
// Compatibility with pre V3.01 versions, renamed functions
//
#define OS_GetType                          OS_INFO_GetLibMode
//
...
//
// Compatibility with pre V3.62 versions, renamed type
//
...
//
// Compatibility with pre V5.8.0 versions, renamed macros
//
#define OS_USEPARA                          OS_USE_PARA
#define OS_ERR_CPU_STATE_ISR_ILLEGAL        OS_ERR_ISR_PRIORITY_INVALID
//
// Compatibility with pre V5.14.0 versions, renamed functions
//
#define OS_GetpCurrentTask()                OS_Global.pCurrentTask
//
// Compatibility with pre V5.16.0 versions, renamed function and types
//
...
//
// Compatibility with pre V5.20.0 versions, renamed types
//
#define OS_PRIO                             OS_TASK_PRIO
