...
public class ImagePaintPlanner
extends AbstractVolatilePaintPlanner {
    protected ImagePaintPlanner(ITheme theme, IPlotItem item, IDomainAxis axis, Rectangle area, int style) {
        super(theme, item, axis, area, style);
    }

    protected ImagePaintPlanner(MultiPaintPlanner multi, IPlotItem item) {
        super(multi, item);
    }
    ...
    protected final void planNormal(IPlan.IPaintPlanGenerator generator, IDomainAxis axis, int idx, long t, int x, long nt) {
        ...
        if (compound != null) {
            ...
            if (!compound.isNone()) {
                ...
                Object value = compound.val();
                if (value instanceof byte[]) {
                    this.lastPaint = 0x7000 | tag;
                    generator.add(this.lastPaint, x, idx, (byte[])value);
                }
                if ((attachments = compound.attachments(12)) != null) {
