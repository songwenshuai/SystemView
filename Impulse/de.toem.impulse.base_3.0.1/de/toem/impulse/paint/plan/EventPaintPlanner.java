...
public class EventPaintPlanner
extends AbstractVolatilePaintPlanner {
    protected EventPaintPlanner(ITheme theme, IPlotItem item, IDomainAxis axis, Rectangle area, int style) {
        super(theme, item, axis, area, style);
    }

    protected EventPaintPlanner(MultiPaintPlanner multi, IPlotItem item) {
        super(multi, item);
    }
    ...
    protected final void planNormal(IPlan.IPaintPlanGenerator generator, IDomainAxis axis, int idx, long t, int x, long nt) {
        ...
        if (compound != null) {
            ...
            if (!compound.isNone()) {
                int v = 0;
                List<Object> member = compound.membersWithContent("event");
                v = member.size() > 0 ? compound.intValueOf(member.get(0)) : compound.intValue();
                String value = compound.format(this.paintStyle.format);
                this.lastPaint = 0x3000 | tag;
                generator.add(this.lastPaint, x, (short)(v &= 0xFFFF), (short)0, idx, value);
