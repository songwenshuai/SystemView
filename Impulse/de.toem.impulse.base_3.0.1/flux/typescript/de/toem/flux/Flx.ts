    ...
    export namespace Flx {
        ...
        export abstract class Buffer {
            ...
            public writeHeadEntry(sformat4 : string, traceId : number, sname : string, sdescription : string, mode : number, maxItemId : number, maxEntrySize : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_HEAD;
                    Flx.arraycopy(format4, 0, this.bytes, written, 4);
                    written += 4;
                    this.bytes[written++] = de.toem.flux.Flx.VERSION;
                    written += Flx.plusWrite(traceId, this.bytes, written);
                    written += Flx.valWrite(name, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(description, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                ...
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeSwitchEntry(traceId : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SWTH;
                    written += Flx.plusWrite(traceId, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            ...
            public writePackEntry(mode : number, compressed : number[], originalSize : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_PBLK;
                    this.bytes[written++] = (<number>mode|0);
                    written += Flx.plusWrite(originalSize, this.bytes, written);
                    written += Flx.valWrite(compressed, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeSectionEntries(noOfSections : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    let contentSize : number = sectionSize - de.toem.flux.Flx.SECTION_HEADER_SIZE;
                    let lastContentSize : number = avail - sectionSize * (noOfSections - 1) - de.toem.flux.Flx.SECTION_HEADER_SIZE;
                    if (lastContentSize < 16 || lastContentSize > 65535){
                        return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
                    }
                    for(let n : number = 0; n < noOfSections; n++) {{
                        this.bytes[written++] = 0;
                        this.bytes[written++] = de.toem.flux.Flx.ENTRY_SECT;
            ...
            public writeScopeDefEntry(itemId : number, parentId : number, sname : string, sdescription : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SCPD;
            ...
            public writeSignalDefEntry(itemId : number, parentId : number, sname : string, sdescription : string, signalType : number, ssignalDescriptor : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SIGD;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.plusWrite(parentId, this.bytes, written);
                    written += Flx.valWrite(name, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(description, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    this.bytes[written++] = (<number>(signalType & 15)|0);
                    written += Flx.valWrite(signalDescriptor, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeMultiSignalDefEntry(itemIdFrom : number, itemIdTo : number, parentId : number, sname : string, sdescription : string, signalType : number, ssignalDescriptor : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_MSGD;
            ...
            public writeSignalReferenceDefEntry(referenceId : number, parentId : number, sname : string, sdescription : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SIRD;
            ...
            public writeScatteredSignalDefEntry(itemId : number, parentId : number, sname : string, sdescription : string, signalType : number, ssignalDescriptor : string, scatteredFrom : number, scatteredTo : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SSGD;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.plusWrite(parentId, this.bytes, written);
                    written += Flx.valWrite(name, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(description, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
            ...
            public writeScatteredSignalReferenceDefEntry(referenceId : number, parentId : number, sname : string, sdescription : string, scatteredFrom : number, scatteredTo : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_SSRD;
                    written += Flx.plusWrite(referenceId, this.bytes, written);
                    written += Flx.plusWrite(parentId, this.bytes, written);
                    written += Flx.valWrite(name, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(description, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
            ...
            public writeOpenEntry(itemId : number, sdomain : string, lstart : number, lrate : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_OPEN;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.valWrite(domain, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(start, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    written += Flx.valWrite(rate, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeCloseEntry(itemId : number, lend : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_CLOS;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.valWrite(end, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeDefaultOpenDomainEntry(sdomain : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_DOMD;
                    written += Flx.valWrite(domain, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            ...
            public writeCurrentEntry(itemId : number, ldomain : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_CURR;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.valWrite(domain, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
            ...
            public writeEnumDefEntry(itemId : number, enumeration : number, slabel : string, value : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_ENMD;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.plusWrite(enumeration, this.bytes, written);
                    written += Flx.valWrite(label, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
            ...
            public writeMemberDefEntry(itemId : number, memberId : number, parentId : number, slabel : string, type : number, sdescriptor : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_MEMD;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    ...
                    written += Flx.valWrite(descriptor, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeRelationEntry(itemId : number, type : number, target : number, style : number, ldeltaOrPosition : number, targetBase : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_ATRE;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    this.bytes[written++] = (<number>type|0);
                    written += Flx.plusWrite(target, this.bytes, written);
                    written += Flx.plusWrite(style, this.bytes, written);
                    written += Flx.valWrite(deltaOrPosition, de.toem.flux.Flx.SZDF_SIZEONLY, this.bytes, written);
            ...
            public writeLabelEntry(itemId : number, style : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
                    this.bytes[written++] = 0;
                    this.bytes[written++] = de.toem.flux.Flx.ENTRY_ATLA;
                    written += Flx.plusWrite(itemId, this.bytes, written);
                    written += Flx.plusWrite(style, this.bytes, written);
            ...
            public writeNoneDataEntry(itemId : number, tag : number, delta : number) : number {
                if (itemId === 0){
                    return de.toem.flux.Flx.ERROR_INVALID_ID;
                }
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    this.bytes[written++] = de.toem.flux.Flx.DF_NONE;
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeIntDataEntry(itemId : number, tag : number, delta : number, value : number) : number {
                ...
                if (vbytes == null){
                    return de.toem.flux.Flx.ERROR_INVALID_VALUE;
                }
                let vlength : number = Flx.length(vbytes);
                let szDf : number = (<number>(de.toem.flux.Flx.DF_DEFAULT | 0)|0);
            ...
            public writeIntArrayDataEntry(itemId : number, tag : number, delta : number, value : any, dynamicSize : boolean) : number {
                ...
                let szDf : number = (<number>((dynamicSize?de.toem.flux.Flx.DF_N_ARRAY:de.toem.flux.Flx.DF_DEFAULT) | Flx.intArrayXdf(value))|0);
            ...
            public writeFloatDataEntry(itemId : number, tag : number, delta : number, value : number) : number {
                ...
                let vlength : number = Flx.length(vbytes);
                let szDf : number = (<number>(de.toem.flux.Flx.DF_DEFAULT | Flx.floatXdf(value))|0);
                let request : number = Flx.plusLen(itemId) + (tag > 1?1:0) + Flx.plusLen(delta) + Flx.plusLen((vlength << 4) | 15) + vlength;
                let started : number = this.request(request);
                if (started >= de.toem.flux.Flx.OK){
                    let written : number = started;
            ...
            public writeFloatArrayDataEntry(itemId : number, tag : number, delta : number, value : any, dynamicSize : boolean) : number {
                ...
                let szDf : number = (<number>((dynamicSize?de.toem.flux.Flx.DF_N_ARRAY:de.toem.flux.Flx.DF_DEFAULT) | Flx.floatArrayXdf(value))|0);
                let addComponentSize : boolean = (szDf & de.toem.flux.Flx.XDF_FLOAT_BIG) === de.toem.flux.Flx.XDF_FLOAT_BIG;
                let vlength : number = Flx.arrayLength(vbytes, dynamicSize, addComponentSize);
                let request : number = Flx.plusLen(itemId) + (tag > 1?1:0) + Flx.plusLen(delta) + Flx.plusLen((vlength << 4) | 15) + vlength;
                let started : number = this.request(request);
                if (started >= de.toem.flux.Flx.OK){
            ...
            public writeEventDataEntry(itemId : number, tag : number, delta : number, value : number) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    written += Flx.valWrite(vbytes, de.toem.flux.Flx.DF_ENUM_EVENT, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeEventArrayDataEntry(itemId : number, tag : number, delta : number, value : number[], dynamicSize : boolean) : number {
                ...
                let szDf : number = (<number>(dynamicSize?de.toem.flux.Flx.DF_N_ARRAY:de.toem.flux.Flx.DF_ENUM_EVENT)|0);
            ...
            public writeTextDataEntry(itemId : number, tag : number, delta : number, value : string) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    written += Flx.valWriteN(vbytes, vlength, de.toem.flux.Flx.DF_DEFAULT, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
            ...
            public writeTextArrayDataEntry(itemId : number, tag : number, delta : number, value : string[], dynamicSize : boolean) : number {
                ...
                let szDf : number = (<number>(dynamicSize?de.toem.flux.Flx.DF_N_ARRAY:de.toem.flux.Flx.DF_DEFAULT)|0);
                let vlength : number = Flx.arrayLength(vbytes, dynamicSize, true);
                let request : number = Flx.plusLen(itemId) + (tag > 1?1:0) + Flx.plusLen(delta) + Flx.plusLen((vlength << 4) | 15) + vlength;
                let started : number = this.request(request);
                if (started >= de.toem.flux.Flx.OK){
            ...
            public writeLogicStatesDataEntry(itemId : number, tag : number, delta : number, precedingStates : number, value : number[], totalBitWidth : number) : number {
                ...
                vlength -= start;
                let stateLevel : number = (maxState >= de.toem.flux.Flx.STATE_Z_BITS?(maxState >= de.toem.flux.Flx.STATE_L_BITS?de.toem.flux.Flx.STATE_LEVEL_16:de.toem.flux.Flx.STATE_LEVEL_4):de.toem.flux.Flx.STATE_LEVEL_2);
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    if (vlength === 0 && precedingStates === de.toem.flux.Flx.STATE_0_BITS){
                        written += Flx.plusWrite(de.toem.flux.Flx.STATE_LEVEL_2 | de.toem.flux.Flx.XDF_LOGIC_PACK_0, this.bytes, written);
                    } else if (vlength === 0 && precedingStates === de.toem.flux.Flx.STATE_1_BITS){
                        written += Flx.plusWrite(de.toem.flux.Flx.STATE_LEVEL_2 | de.toem.flux.Flx.XDF_LOGIC_PACK_1, this.bytes, written);
                    } else {
                        written += Flx.plusWrite((dlength << 4) | stateLevel | de.toem.flux.Flx.XDF_LOGIC_PACK_RIGHT_ALLIGNED, this.bytes, written);
                        let fill : number = 0;
                        let fromBit : number = 0;
                        let toBit : number = 0;
                        if (stateLevel === de.toem.flux.Flx.STATE_LEVEL_2){
                            fill = (<number>de.toem.flux.Flx.LOGIC_L2_BYTE_FILL_$LI$()[precedingStates]|0);
                        ...
                        } else if (stateLevel === de.toem.flux.Flx.STATE_LEVEL_4){
                            fill = (<number>de.toem.flux.Flx.LOGIC_L4_BYTE_FILL_$LI$()[precedingStates]|0);
                        ...
                        } else if (stateLevel === de.toem.flux.Flx.STATE_LEVEL_16){
                            fill = (<number>de.toem.flux.Flx.LOGIC_L16_BYTE_FILL_$LI$()[precedingStates]|0);
            ...
            public writeMemberDataEntry(itemId : number, tag : number, delta : number, value : Flx.MemberValue[]) : number {
                ...
                if (started >= de.toem.flux.Flx.OK){
                    ...
                    written += Flx.memberValWrite(value, vlength, de.toem.flux.Flx.DF_DEFAULT, this.bytes, written);
                    return this.commit(written - started);
                }
                return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
            }
        ...
        export class MemberValue {
            ...
            public pack() : number {
                ...
                let baseType : number = (<number>(this.type & de.toem.flux.Flx.STRUCT_TYPE_MASK_BASE)|0);
                if (baseType === de.toem.flux.Flx.STRUCT_TYPE_GLOBAL_ENUM || baseType === de.toem.flux.Flx.STRUCT_TYPE_LOCAL_ENUM || baseType === de.toem.flux.Flx.STRUCT_TYPE_MERGE_ENUM || baseType === de.toem.flux.Flx.STRUCT_TYPE_INTEGER){
                    bytes = ((typeof this.value === 'number')?Flx.intBytes(<number>this.value):null);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_FLOAT){
                    bytes = ((typeof this.value === 'number')?Flx.floatBytes(<number>this.value):null);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_TEXT){
                    bytes = ((typeof this.value === 'string')?Flx.stringBytes(<string>this.value):null);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_BINARY){
                ...
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_INTEGER_ARRAY){
                    abytes = Flx.intArrayBytes(this.value);
                    xdf = (<number>((Flx.intArrayXdf(this.value) << 2) & de.toem.flux.Flx.STRUCT_MASK_XDF)|0);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_ENUM_ARRAY){
                    abytes = Flx.intArrayBytes(this.value);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_FLOAT_ARRAY){
                    abytes = Flx.floatArrayBytes(this.value);
                    xdf = Flx.floatArrayXdf(this.value);
                    addComponentSize = xdf === de.toem.flux.Flx.XDF_FLOAT_BIG;
                    xdf = (<number>((xdf << 2) & de.toem.flux.Flx.STRUCT_MASK_XDF)|0);
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_TEXT_ARRAY){
                ...
                } else if (baseType === de.toem.flux.Flx.STRUCT_TYPE_STRUCT){
                    if (this.value != null && this.value instanceof <any>Array && (this.value.length==0 || this.value[0] == null ||this.value[0] != null && this.value[0] instanceof <any>de.toem.flux.Flx.MemberValue)){
                        let vlength : number = 0;
                        ...
                        Flx.memberValWrite(members, vlength, de.toem.flux.Flx.SZDF_NONE_$LI$(), bytes, 0);
                        addArraySize = false;
                    }
                }
                if (abytes != null){
                    ...
                    bytes = (s => { let a=[]; while(s-->0) a.push(0); return a; })(vlength);
                    Flx.arrayValWrite(abytes, vlength, de.toem.flux.Flx.SZDF_NONE_$LI$(), bytes, 0, addArraySize, addComponentSize);
                }
                if (bytes != null){
        ...
        export class SimpleBuffer extends Flx.Buffer {
            ...
            public request(len : number) : number {
                if (this.avail() < len){
                    if (this.flush() !== de.toem.flux.Flx.OK){
                        return de.toem.flux.Flx.ERROR_BUFFER_NOT_AVAIL;
                    }
                }
                if (this.avail() < len){
            ...
            public commit(len : number) : number {
                if (this.avail() < len){
                    return de.toem.flux.Flx.ERROR_BUFFER_OVERFLOW;
                }
                this.pos += len;
                return de.toem.flux.Flx.OK;
            }
         ...
         * @param {de.toem.flux.Flx.Buffer} buffer
         * @class
         * @extends de.toem.flux.Flx.TraceItem
         */
        export class Trace extends Flx.TraceItem {
            ...
            public constructor(traceId : number, maxItemId : number, maxEntrySize : number, multiOpen : boolean, buffer : Flx.Buffer) {
                ...
                this.current = 0;
                this.openState = de.toem.flux.Flx.ITEM_OPEN_NONE;
                if (this.items != null){
                    for(let n : number = 0; n < maxItemId; n++) {{
                        this.items[n].type = de.toem.flux.Flx.ITEM_TYPE_UNDEFINED;
                        this.items[n].parentId = 0;
                        this.items[n].openState = de.toem.flux.Flx.ITEM_OPEN_NONE;
            ...
            public setBuffer(buffer : Flx.Buffer) : number {
                if (buffer != null && buffer.trace != null && buffer.trace !== this){
                    return de.toem.flux.Flx.ERROR_BUFFER_ALLREADY_USED;
                }
                if (this.buffer != null){
            ...
            public addHead(name : string, description : string) : number {
                if (this.buffer == null){
                    return de.toem.flux.Flx.ERROR_NO_BUFFER;
                }
                return this.buffer.writeHeadEntry("flux", this.id, name, description, (<number>de.toem.flux.Flx.MODE_HEAD_NORMAL|0), this.maxItemId, this.maxEntrySize);
            }
            ...
            public addHeadDerived(format4 : string, name : string, description : string) : number {
                ...
                return this.buffer.writeHeadEntry(format4, this.id, name, description, (<number>de.toem.flux.Flx.MODE_HEAD_NORMAL|0), this.maxItemId, this.maxEntrySize);
            }
            ...
            public addScope(itemId : number, parentId : number, name : string, description : string) : number {
                ...
                if (this.items != null){
                    if (this.items[itemId - 1].type !== de.toem.flux.Flx.ITEM_TYPE_UNDEFINED){
                        return de.toem.flux.Flx.ERROR_ITEM_ALLREADY_DEFINED;
                    }
                    if (parentId !== 0 && this.items[parentId - 1].type !== de.toem.flux.Flx.ITEM_TYPE_SCOPE){
                        return de.toem.flux.Flx.ERROR_PARENT_NOT_DEFINED;
                    }
                    this.items[itemId - 1].type = de.toem.flux.Flx.ITEM_TYPE_SCOPE;
                    this.items[itemId - 1].openState = de.toem.flux.Flx.ITEM_OPEN_NONE;
            ...
            public addSignal(itemId : number, parentId : number, name : string, description : string, signalType : number, signalDescriptor : string) : number {
                ...
                if (this.items != null){
                    ...
                    this.items[itemId - 1].type = de.toem.flux.Flx.ITEM_TYPE_SIGNAL;
                    this.items[itemId - 1].openState = de.toem.flux.Flx.ITEM_OPEN_NONE;
                    this.items[itemId - 1].parentId = parentId;
                }
            ...
            public addSignalReference(referenceId : number, parentId : number, name : string, description : string) : number {
                ...
                if (this.items != null){
                    if (this.items[referenceId - 1].type !== de.toem.flux.Flx.ITEM_TYPE_SIGNAL){
                        return de.toem.flux.Flx.ERROR_ITEM_NOT_DEFINED;
                    }
                    if (parentId !== 0 && this.items[parentId - 1].type !== de.toem.flux.Flx.ITEM_TYPE_SCOPE){
                        return de.toem.flux.Flx.ERROR_PARENT_NOT_DEFINED;
                    }
            ...
            public isSignal(itemId : number) : boolean {
                return this.items != null && this.items[itemId - 1].type === de.toem.flux.Flx.ITEM_TYPE_SIGNAL;
            }
            ...
            public isScope(itemId : number) : boolean {
                return this.items != null && this.items[itemId - 1].type === de.toem.flux.Flx.ITEM_TYPE_SCOPE;
            }
            ...
            public open(itemId : number, domainBase : string, start : number, rate : number) : number {
                ...
                if (itemId === 0){
                    if (this.openState !== de.toem.flux.Flx.ITEM_OPEN_NONE){
                        return de.toem.flux.Flx.ERROR_ALLREADY_OPEN;
                    }
                } else {
                    if (this.items[itemId - 1].openState !== de.toem.flux.Flx.ITEM_OPEN_NONE){
                        return de.toem.flux.Flx.ERROR_ALLREADY_OPEN;
                    }
                }
                if (this.items != null){
                    for(let n : number = 1; n < this.maxItemId; n++) {{
                        if (this.items[n - 1].openState !== de.toem.flux.Flx.ITEM_OPEN_NONE){
                            let p : number = this.items[itemId - 1].parentId;
                            while((true)) {{
                                if (p === itemId){
                                    return de.toem.flux.Flx.ERROR_CHILDREN_ALLREADY_OPEN;
                                }
                                if (p === 0){
                                    break;
                                }
                ...
                if (itemId === 0){
                    this.openState = de.toem.flux.Flx.ITEM_OPEN_LOCAL;
                    this.current = start;
                } else {
                    this.items[itemId - 1].openState = de.toem.flux.Flx.ITEM_OPEN_LOCAL;
                ...
                if (this.items != null){
                    for(let n : number = 1; n < this.maxItemId; n++) {{
                        ...
                        while((true)) {{
                            if (p === itemId){
                                this.items[n - 1].openState = de.toem.flux.Flx.ITEM_OPEN_CONTAINER;
            ...
            public close(itemId : number, end : number) : number {
                ...
                if (itemId === 0){
                    if (this.openState !== de.toem.flux.Flx.ITEM_OPEN_LOCAL){
                        return de.toem.flux.Flx.ERROR_NOT_OPEN;
                    }
                    current = this.current;
                } else {
                    if (this.items[itemId - 1].openState !== de.toem.flux.Flx.ITEM_OPEN_LOCAL){
                        return de.toem.flux.Flx.ERROR_NOT_OPEN;
                    }
                ...
                if (this.items != null){
                    for(let n : number = 1; n < this.maxItemId; n++) {{
                        ...
                        while((true)) {{
                            if (p === itemId){
                                this.items[n - 1].openState = de.toem.flux.Flx.ITEM_OPEN_NONE;
            ...
            public isOpen(itemId : number) : boolean {
                return (this.openState === de.toem.flux.Flx.ITEM_OPEN_LOCAL) || (this.items != null && this.items[itemId - 1].openState !== de.toem.flux.Flx.ITEM_OPEN_NONE);
            }
            ...
            public getCurrent(itemId : number) : number {
                ...
                if (this.openState === 0 && this.items != null){
                    if (this.items[itemId - 1].openState === de.toem.flux.Flx.ITEM_OPEN_LOCAL){
                        openId = itemId;
                    } else if (this.items[itemId - 1].openState === de.toem.flux.Flx.ITEM_OPEN_CONTAINER){
                        openId = this.items[itemId - 1].openId;
                        if (this.items[openId - 1].openState !== de.toem.flux.Flx.ITEM_OPEN_LOCAL){
                            return de.toem.flux.Flx.ERROR_NOT_OPEN;
                        }
                    } else {
                        return de.toem.flux.Flx.ERROR_NOT_OPEN;
            ...
            public writeArrayDef(itemId : number, index : number, label : string, memberDescriptor : string) : number {
                ...
                return this.buffer.writeMemberDefEntry(itemId, index, -1, label, de.toem.flux.Flx.STRUCT_TYPE_UNKNOWN, memberDescriptor);
            }
            ...
            getOpenItem(itemId : number) : Flx.TraceItem {
                if (this.openState === de.toem.flux.Flx.ITEM_OPEN_NONE && this.items != null){
                    let openId : number = 0;
                    if (this.items[itemId - 1].openState === de.toem.flux.Flx.ITEM_OPEN_LOCAL){
                        openId = itemId;
                    } else if (this.items[itemId - 1].openState === de.toem.flux.Flx.ITEM_OPEN_CONTAINER){
            ...
            public writeCurrent(itemId : number, domainPosition : number) : number {
                ...
                if (delta < 0){
                    return de.toem.flux.Flx.ERROR_POSITION_LESSTHAN_CURRENT;
                }
...
de.toem.flux.Flx.CHAR_2_STATE_$LI$();

de.toem.flux.Flx.STATE_UC_DIGITS_$LI$();

de.toem.flux.Flx.STATE_LC_DIGITS_$LI$();

de.toem.flux.Flx.LOGIC_L16_BYTE_FILL_$LI$();

de.toem.flux.Flx.LOGIC_L4_BYTE_FILL_$LI$();

de.toem.flux.Flx.LOGIC_L2_BYTE_FILL_$LI$();

de.toem.flux.Flx.SZDF_NONE_$LI$();
