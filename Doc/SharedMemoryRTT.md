...
- RTT control block 中的 `sName` 和 `pBuffer` 字段存储固定 64 位相对 offset，而不是标准 SEGGER RTT 使用的目标地址绝对指针。
...
本工程面向异构系统中的 RTT 数据交换。R 核和 A 核通过一段共享内存访问同一个 SEGGER RTT control block 和 ring buffer。共享内存必须映射为 uncache，双方都可以直接进行 32/64 位 volatile 读写和 payload `memcpy` 访问。
...
每个 up/down buffer descriptor 为 `SEGGER_RTT__BUFFER_SIZE` 字节，当前布局为 32 字节，字段包括：
...
`sName` 和 `pBuffer` 存储的是相对 RTT control block 基地址的固定 64 位 offset，不是绝对指针。每一方访问 name 或 ring-buffer payload 时，都使用本地映射基地址计算：
...
- `Address` 必须是 RTT control block 起始地址，并且必须 8 字节对齐。
...
- 任意 `sName - Address`、`pBuffer - Address`、`pBuffer - Address + BufferSize` 都必须能用 64 位无符号 offset 表示，并且必须能被当前访问方的本地地址空间安全访问。
...
当前 RTT 提供 `SEGGER_RTT_FindControlBlock(&Address, Size)` 和 `SEGGER_RTT_FindValidControlBlock(&Address, Size, &RegionSize)` 用于这个场景。调用前 `Address` 是共享内存映射起始地址，`Size` 是映射范围大小；找到后 `Address` 会被更新为 RTT control block 起始地址。`SEGGER_RTT_FindControlBlock()` 只搜索 RTT ID，不会初始化或修改 control block，并且只接受 8 字节对齐且至少能容纳 RTT 基础头部的候选地址。`SEGGER_RTT_FindValidControlBlock()` 会继续校验 control block、descriptor table 和已配置 payload buffer 是否位于返回地址之后的剩余映射范围内。

读取 down-buffer 时，`aDown[]` 的起始地址必须用共享内存中的 `MaxNumUpBuffers` 运行时计算：

```c
aDownBase = Address + SEGGER_RTT__CB_OFF_A_UP + MaxNumUpBuffers * SEGGER_RTT__BUFFER_SIZE;
```
...
- `SEGGER_RTT_CheckInit(Address)` 会检查 control block 地址非空、8 字节对齐且 ID 完整匹配。
- `SEGGER_RTT_InitEx(Address, Size)` 会在清零初始化前检查 `Size >= SEGGER_RTT__REQUIRED_MEM_SIZE`。
- `SEGGER_RTT_EnsureInitEx(Address, Size, NumBuffers)` 会在需要时初始化 control block，或校验已存在 control block，然后按共享内存布局配置请求的额外 up/down buffer pair。
- `SEGGER_RTT_CheckRegion(Address, Size)` 会验证完整 ID、runtime buffer count、descriptor table 范围、已配置 payload 范围、`RdOff/WrOff` 和 flags。
- `SEGGER_RTT_CheckUpBuffer(Address, Size, BufferIndex)` 会验证指定 up-buffer descriptor 位于映射区域内并且已经配置。
- `SEGGER_RTT_CheckDownBuffer(Address, Size, BufferIndex)` 会验证指定 down-buffer descriptor 位于映射区域内并且已经配置。
- `SEGGER_RTT_FindControlBlock(&Address, Size)` 只在调用方传入的映射范围内搜索，并拒绝非 8 字节对齐或连基础头部都放不下的候选地址。
- `SEGGER_RTT_FindValidControlBlock(&Address, Size, &RegionSize)` 会在搜索到 ID 后继续校验该候选 control block 对剩余映射范围有效。
...
- `Config*`、`Alloc*`、`SetFlags*` 会拒绝无效 flags、过小 buffer 和不能编码成 64 位相对 offset 或不能被本地安全访问的 name/buffer 地址。
...
- `SEGGER_RTT_EnsureInitEx(Address, Size, NumBuffers)`：初始化或复用 RTT control block，并按映射区域配置额外 up/down buffer pair。
...
- `SEGGER_RTT_CheckUpBuffer(Address, Size, BufferIndex)`：只校验指定 up-buffer descriptor 是否有效且已配置，不会修改 control block。
- `SEGGER_RTT_CheckDownBuffer(Address, Size, BufferIndex)`：只校验指定 down-buffer descriptor 是否有效且已配置，不会修改 control block。
- `SEGGER_RTT_FindControlBlock(&Address, Size)`：在共享内存映射范围内搜索 RTT control block，找到后更新 `Address`。
- `SEGGER_RTT_FindValidControlBlock(&Address, Size, &RegionSize)`：在共享内存映射范围内搜索有效 RTT control block，找到后更新 `Address` 和剩余 region size。
...
- 共享内存必须是 uncache 映射。
- 共享内存基地址必须满足 8 字节对齐，保证 64 位 descriptor offset field 访问安全。
- descriptor 的 `sName` 和 `pBuffer` 使用 64 位读写，其他计数和 ring 状态字段使用 32 位读写。
...
- 调用配置 API 传入的 `sName` 和 `pBuffer` 必须位于同一个共享内存映射内，并且能用相对 RTT control block 基地址的 64 位 offset 表示。
...
- 如果 control block 可能被另一个核部分写入、复位中断打断或被错误写坏，访问方必须先用 `SEGGER_RTT_CheckInit(Address)`、`SEGGER_RTT_FindControlBlock(&Address, Size)` 或 `SEGGER_RTT_FindValidControlBlock(&Address, Size, &RegionSize)` 建立完整 ID 前置条件，再进入普通读写路径。
